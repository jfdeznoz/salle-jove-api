...
`+result.renderMismatch())}hasNone(){this.hasValues(match_1().Match.exact({}))}all(){return this._tags}}exports.Tags=Tags,_a=JSII_RTTI_SYMBOL_1,Tags[_a]={fqn:"aws-cdk-lib.assertions.Tags",version:"2.154.1"};function getManifestTags(stack){const root=stack.node.root;if(!core_1().Stage.isStage(root))throw new Error("unexpected: all stacks must be part of a Stage or an App");return root.synth().getStackArtifact(stack.artifactId).tags}
